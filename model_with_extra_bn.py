...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.bn1_adv = nn.BatchNorm1d(128)
        self.bn2_adv = nn.BatchNorm1d(256)
        ...
        self.bn3_adv = nn.BatchNorm1d(512)
        self.bn4_adv = nn.BatchNorm1d(1024)
    
    def forward(self, x, npts, is_adv):
        n = x.size()[2]

        if is_adv:   
            # first shared mlp
            x = F.relu(self.bn1_adv(self.conv1(x)))
            f = self.bn2_adv(self.conv2(x))     
        else:
            # first shared mlp
        ...
        # point-wise maxpool
        g = point_wise_maxpool(f, npts, keepdims=True) 
        
        # expand and concat
        gs = expand(g, npts)
        x = torch.cat([gs, f], dim=1)                  

        if is_adv:
            # second shared mlp
            x = F.relu(self.bn3_adv(self.conv3(x)))
            x = self.bn4_adv(self.conv4(x))
        else:
            # second shared mlp
        ...
        # point-wise maxpool
        v = point_wise_maxpool(x, npts, keepdims=False)
...
class Decoder(nn.Module):
    def __init__(self, num_coarse=1024, num_dense=16384):
        ...
        self.bn1_adv = nn.BatchNorm1d(1024)
        self.bn2_adv = nn.BatchNorm1d(1024)
        ...
        self.bn3_adv = nn.BatchNorm1d(512)
        self.bn4_adv = nn.BatchNorm1d(512)
    ...
    def forward(self, x, is_adv):
        ...
        if is_adv:
            x = F.relu(self.bn1_adv(self.linear1(x)))
            x = F.relu(self.bn2_adv(self.linear2(x)))
        else:
        ...
        if is_adv:
            x = F.relu(self.bn3_adv(self.conv1(x)))
            x = F.relu(self.bn4_adv(self.conv2(x)))
        else:
...
class AutoEncoderBN(nn.Module):
    ...
    def forward(self, x, npts, is_adv):
        v = self.encoder(x, npts, is_adv)
        y_coarse, y_detail = self.decoder(v, is_adv)
