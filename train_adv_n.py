...
import random
from model_bn import AutoEncoder
from loss import ChamferDistance, EarthMoverDistance
from dataset.dataset import ShapeNet, ShapeNetAdvN, collate_fn, collate_fn_adv
from attack_normal import RepresentationAdv

# Logging File Created
logging.basicConfig(filename='myrun.log', level=logging.INFO)
...
def train_(local_rank, args):
    ...
    train_dataset = ShapeNetAdvN(True, args.train_path, args.adv_path, args.pd_path, args.num_input, 
    ...
    for epoch in range(19, args.epochs + 1):
        ...
        for data in train_dataloader:
            ...
            ids, npts, partial_input, partial_input_adv, normal, dense_gt, coarse_gt = data
            ...
            if args.adv:
                ...
                normal = normal.cuda(local_rank, non_blocking=True)
    
                new_input_adv = Rep.get_loss(npts, partial_input_adv, coarse_gt, dense_gt, normal, 
            network, loss_d1, random_start=False)

                temp = torch.split(new_input_adv, npts, dim=2)
                for i, index in enumerate(ids):
                    torch.save(temp[i].cpu(), args.adv_path + 'adv_'+str(index)+'.pt')
...
def main():
    ...
    parser.add_argument('--adv_path', type=str, default='/scratch/mw4355/shapenet/adv_saved/')
    parser.add_argument('--pd_path', type=str, default='/scratch/mw4355/shapenet/normals/')
    ...
    parser.add_argument('--save_name', type=str, default='adv_bn_proj.pth')
